"""
    ParaBART model from https://github.com/uclanlp/ParaBART
    Paper: Disentangling Semantics and Syntax in Sentence Embeddings with Pre-trained Language Models
    Authors: Huang, James Y. and Huang, Kuan-Hao and Chang, Kai-Wei
 """
...
class ParaBart(PretrainedBartModel):
    ...
    def forward(
        ...
        attention_mask2 = torch.cat((torch.zeros(input_ids.shape[0], 1).to(self.device).bool(), attention_mask[:, self.config.max_sent_len+2:]), dim=1)
...
class ParaBartEncoder(nn.Module):
    def __init__(self, config, embed_tokens):
        ...
        self.embed_positions = LearnedPositionalEmbedding(
            config.max_position_embeddings, config.d_model, config.pad_token_id, 2
        )
    ...
    def forward_token(self, input_token_ids, attention_mask):
        #if self.training:
        #    drop_mask = torch.bernoulli(self.config.word_dropout*torch.ones(input_token_ids.shape)).bool().to(torch.device('mps'))
